...
public class PersistenceService implements InterfacePersistencia {
    ...
    private RuntimeTypeAdapterFactory<Produto> produtoAdapter =
    ...
    public Config carregarConfig() {
        ...
        if (configFile.exists()) {
            ...
            try (Reader reader = new InputStreamReader(
                ...
                System.out.println(
                        "Configurações carregadas de " + CONFIG_FILE
                );
                ...
                System.out.println(
                        "Erro ao ler config.json, usando padrão. Erro: "
                                + e.getMessage()
                );
            ...
            System.out.println(
                    CONFIG_FILE
                            + " não encontrado, criando um novo com valores padrão."
            );
    ...
    public void salvarProdutos(List<Produto> produtos) {
        ...
        try (Writer writer = new OutputStreamWriter(
                new FileOutputStream(PRODUTOS_FILE),
                StandardCharsets.UTF_8
        )) {
            ...
            System.out.println(
                    "Produtos salvos em " + PRODUTOS_FILE
            );

        } catch (IOException e) {

            throw new PersistenciaException(
                    "Não foi possível salvar os itens do cardápio.",
                    e
            );
        }
    }
    ...
    public List<Produto> carregarProdutos() {
        ...
        if (!produtoFile.exists()) {
            ...
            System.out.println(
                    PRODUTOS_FILE
                            + " não encontrado, retornando lista vazia."
            );
        ...
        try (Reader reader = new InputStreamReader(
                new FileInputStream(produtoFile),
                StandardCharsets.UTF_8
        )) {
            ...
            System.out.println(
                    "Produtos carregados de " + PRODUTOS_FILE
            );

            if (precisouMigrar) {

                System.out.println(
                        "Migração de dados antigos detectada. "
                                + "Atualizando arquivo..."
                );
    ...
    private String inferirTipoClasse(JsonObject produto) {
        ...
        String categoria =
                produto.get("categoriaNome")
                        .getAsString()
                        .toLowerCase();

        if (categoria.contains("sem")
                && categoria.contains("lcool")) {

            return "sem_alcool";
        }

        if (categoria.contains("com")
                && categoria.contains("lcool")) {
    ...
    public List<Usuario> carregarUsuarios() {
        ...
        File usuariosFile =
                new File(USUARIOS_FILE);

        if (usuariosFile.exists()) {

            try (Reader reader = new InputStreamReader(
                    new FileInputStream(usuariosFile),
                    StandardCharsets.UTF_8
            )) {
    ...
    public void salvarUsuarios(List<Usuario> usuarios) {
        ...
        try (Writer writer = new OutputStreamWriter(
                new FileOutputStream(USUARIOS_FILE),
                StandardCharsets.UTF_8
        )) {
            ...
            System.out.println(
                    "Usuários salvos em " + USUARIOS_FILE
            );
